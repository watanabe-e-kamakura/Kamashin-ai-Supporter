...
- `.company/guardian/cross-repo-impact-map.md` — クロスリポ影響マップ（Phase 0 の impact-analyzer に渡す）
...
- `cross-repo-impact-map.md` → Phase 0 の impact-analyzer エージェント専用
...
## autodev: UI検証
（Phase 2.5 完了後に追記 — 対象画面・スクリーンショット・判定結果。バックエンドのみの場合はスキップ）
...
- `.company/guardian/cross-repo-impact-map.md`（影響パターン8種+チェックリスト）
- 対象リポジトリのコード構造（関連ファイル一覧）
- `itns-documents/tasks-division-cross-dev/overview/system-architecture.md`（依存グラフ、存在する場合）

**impact-analyzer が出力するもの:**
1. **変更種別**: Type 1〜8 のどれに該当するか
2. **影響リポジトリ一覧**: 主要 / 波及 / 要確認 の3段階
3. **配置判断**: 「共通 vs 事業部」の推奨（判断が必要な場合）
...
- **Issue番号なし**: `.company/secretary/notes/` にフォールバック
...
**発動条件**: 変更ファイルに `.blade.php`, `.vue`, `.css`, `.scss`, `.ts`（UI関連）が含まれる場合のみ実行する。バックエンドのみの変更はスキップ。

### 2.5-1. UI検証の準備

1. `references/playwright-guide.md` を読み込む
2. 変更したBladeファイル・Vueコンポーネントから対象画面のURLを特定する
3. Dockerコンテナ（またはworktreeコンテナ）が起動しているか確認する

### 2.5-2. Playwright MCPでスクリーンショット取得

Playwright MCPツールを使い、変更画面のスクリーンショットを取得する。

1. ブラウザでログイン（テストユーザー情報は `src/web/database/seeders/` から取得）
2. 変更した画面に遷移
3. `browser_screenshot` でスクリーンショットを取得・保存
4. 必要に応じて操作後の状態（フォーム入力後、バリデーションエラー表示等）もキャプチャ

**保存先:** `.playwright-mcp/screenshots/{リポジトリ名}/`
**命名規則:** `{リポジトリ名}-{画面名}-{状態}.png`

### 2.5-3. 目視確認

Claude がスクリーンショットを確認し、以下をチェック:
- レイアウト崩れがないか
- テキストの切れ・はみ出しがないか
- 変更箇所が設計通りに反映されているか
...
### 2.5-4. スクリーンショットをPR用に保持

- `.playwright-mcp/screenshots/` にスクリーンショットを保持する（PR作成時に使用）
- 最終報告の「UI検証」セクションに結果を記載する
...
- **静的解析準拠**: `Model::find()` 等の戻り値には `/** @var Model|null */` のPHPDocを付与し、PHPStanエラーを出さない。実装時に型アノテーションを省略しない
